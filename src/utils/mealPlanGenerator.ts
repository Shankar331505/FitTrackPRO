...
export function calculateOptimalMacros(input: GoalInput): NutritionGoals {
    ...
    if (weightDiff < -2) {
        const weeklyDeficit = Math.abs(weeklyWeightChange) * 7700;
        const dailyDeficit = weeklyDeficit / 7;
        calories = Math.max(tdee - dailyDeficit, bmr * 1.2);
        proteinPerKg = 2.2;
        fatPercentage = 25;
...
export async function generateMealPlan(goals: NutritionGoals): Promise<MealPlanDay> {
    ...
    const totalFoods =
        mealPlan.breakfast.length + mealPlan.lunch.length +
        mealPlan.dinner.length + mealPlan.snacks.length;
    if (totalFoods === 0) {
    ...
    // Adjustment pass
    if (allFoods.length > 0 && mealPlan.totals.calories > 0) {
        ...
        if (adjustmentRatio > 0.7 && adjustmentRatio < 1.3) {
            allFoods.forEach(entry => {
                entry.quantity = Math.max(20, Math.min(400, Math.round(entry.quantity * adjustmentRatio)));
            });
            mealPlan.totals = allFoods.reduce(
                (acc, entry) => {
                    const m = entry.quantity / 100;
...
export function generateSimpleMealPlan(goals: NutritionGoals): MealPlanDay {
    ...
    // Scale food quantities to meet calorie goals
    const scale = goals.calories / 1850; // baseline ~1850 kcal

    const qty = (base: number) => Math.max(20, Math.round(base * scale));

    const breakfast: FoodEntry[] = [
        { food: simpleFoods.roti as any, quantity: qty(100), mealType: 'breakfast' },
        { food: simpleFoods.egg as any, quantity: qty(100), mealType: 'breakfast' },
        { food: simpleFoods.milk as any, quantity: qty(200), mealType: 'breakfast' },
        { food: simpleFoods.banana as any, quantity: qty(100), mealType: 'breakfast' },
    ];
    const lunch: FoodEntry[] = [
        { food: simpleFoods.rice as any, quantity: qty(150), mealType: 'lunch' },
        { food: simpleFoods.dal as any, quantity: qty(150), mealType: 'lunch' },
        { food: simpleFoods.chicken as any, quantity: qty(150), mealType: 'lunch' },
        { food: simpleFoods.roti as any, quantity: qty(70), mealType: 'lunch' },
    ];
    const dinner: FoodEntry[] = [
        { food: simpleFoods.roti as any, quantity: qty(120), mealType: 'dinner' },
        { food: simpleFoods.paneer as any, quantity: qty(100), mealType: 'dinner' },
        { food: simpleFoods.dal as any, quantity: qty(100), mealType: 'dinner' },
    ];
    const snacks: FoodEntry[] = [
        { food: simpleFoods.almonds as any, quantity: 30, mealType: 'snacks' },
        { food: simpleFoods.banana as any, quantity: qty(100), mealType: 'snacks' },
    ];
